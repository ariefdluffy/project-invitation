...
import { t as fix_stack_trace } from "./shared-server.js";
			...
			*/
			const as_func = (type, input_value) => {
				...
				if (type === "submit" || type === "hidden") {
					if (!input_value) throw new Error(`\`${type}\` inputs must have a value`);
...
async function handle_error_and_jsonify(event, state, options, error) {
	...
	if (typeof error == "object") fix_stack_trace(error);
	const status = get_status(error);
