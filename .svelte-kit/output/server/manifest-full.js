export const manifest = (() => {
function __memo(fn) {
	let value;
	return () => value ??= (value = fn());
}

return {
	appDir: "_app",
	appPath: "_app",
	assets: new Set(["favicon.ico","qris_dummy.png","robots.txt","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-37-07--1--1777084795947.jpeg","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-37-07-1777084801661.jpeg","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-37-08--1--1777084811542.jpeg","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-37-08--2--1777084830439.jpeg","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-37-08-1777084840272.jpeg","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-37-09-1777084818384.jpeg","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-43-54-1777085568239.jpeg","uploads/28d6f5a2-8033-474f-a185-4c64fe0c5f39/whatsapp-image-2026-04-25-at-10-43-55-1777085562999.jpeg","uploads/33e01e0f-a672-4dc4-bc90-c53363b52723-bride_photo-1777085679984.jpeg","uploads/33e01e0f-a672-4dc4-bc90-c53363b52723-groom_photo-1777085679985.jpeg","uploads/46c9a186-47e3-49f5-b9cb-fa8bdc25c1ab-background_image-1777046991096.jpg","uploads/46c9a186-47e3-49f5-b9cb-fa8bdc25c1ab-bride_photo-1777047022777.jpg","uploads/46c9a186-47e3-49f5-b9cb-fa8bdc25c1ab-gallery_images-1777046991099.jpg","uploads/46c9a186-47e3-49f5-b9cb-fa8bdc25c1ab-groom_photo-1777047022778.jpg","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/486547023-1065802202248954-5082179717334344134-n-1777038976007.jpg","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/486609158-1065802175582290-5325662359838097259-n-1777039923203.jpg","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/678633183-3861485243982574-7258817377620287722-n-1777040867728.jpg","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/678924860-2736906306696415-5379513880898333742-n-1777040779330.jpg","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/679888091-3861484733982625-789921743050444292-n-1777040831714.jpg","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/channels4-profile-1777038547553.jpg","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/futuristic-armored-agent-in-flames-1777037523157.png","uploads/9b787bdb-bc47-4ba6-b552-01a04f92e022/gemini-generated-image-avs2vgavs2vgavs2-1777037513459.png"]),
	mimeTypes: {".png":"image/png",".txt":"text/plain",".jpeg":"image/jpeg",".jpg":"image/jpeg"},
	_: {
		client: {start:"_app/immutable/entry/start.BjxTFjGV.js",app:"_app/immutable/entry/app.BB4W0U4Q.js",imports:["_app/immutable/entry/start.BjxTFjGV.js","_app/immutable/chunks/Dmxo6f7P.js","_app/immutable/chunks/DdYSuJ8G.js","_app/immutable/entry/app.BB4W0U4Q.js","_app/immutable/chunks/DdYSuJ8G.js","_app/immutable/chunks/TWoP20JI.js","_app/immutable/chunks/3bfzTCuz.js"],stylesheets:[],fonts:[],uses_env_dynamic_public:false},
		nodes: [
			__memo(() => import('./nodes/0.js')),
			__memo(() => import('./nodes/1.js')),
			__memo(() => import('./nodes/2.js')),
			__memo(() => import('./nodes/3.js')),
			__memo(() => import('./nodes/4.js')),
			__memo(() => import('./nodes/5.js')),
			__memo(() => import('./nodes/6.js')),
			__memo(() => import('./nodes/7.js')),
			__memo(() => import('./nodes/8.js')),
			__memo(() => import('./nodes/9.js')),
			__memo(() => import('./nodes/10.js')),
			__memo(() => import('./nodes/11.js')),
			__memo(() => import('./nodes/12.js')),
			__memo(() => import('./nodes/13.js')),
			__memo(() => import('./nodes/14.js')),
			__memo(() => import('./nodes/15.js')),
			__memo(() => import('./nodes/16.js')),
			__memo(() => import('./nodes/17.js')),
			__memo(() => import('./nodes/18.js')),
			__memo(() => import('./nodes/19.js')),
			__memo(() => import('./nodes/20.js')),
			__memo(() => import('./nodes/21.js')),
			__memo(() => import('./nodes/22.js'))
		],
		remotes: {
			
		},
		routes: [
			{
				id: "/",
				pattern: /^\/$/,
				params: [],
				page: { layouts: [0,], errors: [1,], leaf: 4 },
				endpoint: null
			},
			{
				id: "/admin",
				pattern: /^\/admin\/?$/,
				params: [],
				page: { layouts: [0,2,], errors: [1,,], leaf: 5 },
				endpoint: null
			},
			{
				id: "/admin/invitations",
				pattern: /^\/admin\/invitations\/?$/,
				params: [],
				page: { layouts: [0,2,], errors: [1,,], leaf: 6 },
				endpoint: null
			},
			{
				id: "/admin/settings",
				pattern: /^\/admin\/settings\/?$/,
				params: [],
				page: { layouts: [0,2,], errors: [1,,], leaf: 7 },
				endpoint: null
			},
			{
				id: "/admin/templates",
				pattern: /^\/admin\/templates\/?$/,
				params: [],
				page: { layouts: [0,2,], errors: [1,,], leaf: 8 },
				endpoint: null
			},
			{
				id: "/admin/templates/preview/[id]",
				pattern: /^\/admin\/templates\/preview\/([^/]+?)\/?$/,
				params: [{"name":"id","optional":false,"rest":false,"chained":false}],
				page: { layouts: [0,2,], errors: [1,,], leaf: 9 },
				endpoint: null
			},
			{
				id: "/admin/users",
				pattern: /^\/admin\/users\/?$/,
				params: [],
				page: { layouts: [0,2,], errors: [1,,], leaf: 10 },
				endpoint: null
			},
			{
				id: "/api/midtrans/create-transaction",
				pattern: /^\/api\/midtrans\/create-transaction\/?$/,
				params: [],
				page: null,
				endpoint: __memo(() => import('./entries/endpoints/api/midtrans/create-transaction/_server.ts.js'))
			},
			{
				id: "/api/midtrans/notification",
				pattern: /^\/api\/midtrans\/notification\/?$/,
				params: [],
				page: null,
				endpoint: __memo(() => import('./entries/endpoints/api/midtrans/notification/_server.ts.js'))
			},
			{
				id: "/dashboard",
				pattern: /^\/dashboard\/?$/,
				params: [],
				page: { layouts: [0,3,], errors: [1,,], leaf: 11 },
				endpoint: null
			},
			{
				id: "/dashboard/billing",
				pattern: /^\/dashboard\/billing\/?$/,
				params: [],
				page: { layouts: [0,3,], errors: [1,,], leaf: 12 },
				endpoint: null
			},
			{
				id: "/dashboard/billing/checkout",
				pattern: /^\/dashboard\/billing\/checkout\/?$/,
				params: [],
				page: { layouts: [0,3,], errors: [1,,], leaf: 13 },
				endpoint: null
			},
			{
				id: "/dashboard/create",
				pattern: /^\/dashboard\/create\/?$/,
				params: [],
				page: { layouts: [0,3,], errors: [1,,], leaf: 14 },
				endpoint: null
			},
			{
				id: "/dashboard/invitations",
				pattern: /^\/dashboard\/invitations\/?$/,
				params: [],
				page: { layouts: [0,3,], errors: [1,,], leaf: 15 },
				endpoint: null
			},
			{
				id: "/dashboard/invitations/[id]",
				pattern: /^\/dashboard\/invitations\/([^/]+?)\/?$/,
				params: [{"name":"id","optional":false,"rest":false,"chained":false}],
				page: { layouts: [0,3,], errors: [1,,], leaf: 16 },
				endpoint: null
			},
			{
				id: "/dashboard/media",
				pattern: /^\/dashboard\/media\/?$/,
				params: [],
				page: { layouts: [0,3,], errors: [1,,], leaf: 17 },
				endpoint: null
			},
			{
				id: "/demo/[id]",
				pattern: /^\/demo\/([^/]+?)\/?$/,
				params: [{"name":"id","optional":false,"rest":false,"chained":false}],
				page: { layouts: [0,], errors: [1,], leaf: 18 },
				endpoint: null
			},
			{
				id: "/invitation/[slug]",
				pattern: /^\/invitation\/([^/]+?)\/?$/,
				params: [{"name":"slug","optional":false,"rest":false,"chained":false}],
				page: { layouts: [0,], errors: [1,], leaf: 19 },
				endpoint: null
			},
			{
				id: "/login",
				pattern: /^\/login\/?$/,
				params: [],
				page: { layouts: [0,], errors: [1,], leaf: 20 },
				endpoint: null
			},
			{
				id: "/logout",
				pattern: /^\/logout\/?$/,
				params: [],
				page: { layouts: [0,], errors: [1,], leaf: 21 },
				endpoint: null
			},
			{
				id: "/register",
				pattern: /^\/register\/?$/,
				params: [],
				page: { layouts: [0,], errors: [1,], leaf: 22 },
				endpoint: null
			},
			{
				id: "/uploads/[...path]",
				pattern: /^\/uploads(?:\/([^]*))?\/?$/,
				params: [{"name":"path","optional":false,"rest":true,"chained":true}],
				page: null,
				endpoint: __memo(() => import('./entries/endpoints/uploads/_...path_/_server.ts.js'))
			}
		],
		prerendered_routes: new Set([]),
		matchers: async () => {
			
			return {  };
		},
		server_assets: {}
	}
}
})();
