...
export async function fixMusicLinks(): Promise<void> {
	...
	try {
		// Hanya update settings default, JANGAN update semua undangan yang sudah ada
		// (user mungkin sudah set music custom mereka sendiri)
		await db.execute(
			"UPDATE settings SET value = ? WHERE `key` = 'default_music_url'",
			[newMusic]
		);
	} catch (err) {
...
export async function seedTemplates(): Promise<void> {
	...
	console.log(`[Server] Found ${fsTemplates.length} templates in filesystem. Syncing to database...`);

	// Gunakan INSERT ... ON DUPLICATE KEY UPDATE (UPSERT) agar tidak TRUNCATE data
	// Ini aman untuk production - tidak menghapus data yang sudah ada
	const upsertQuery = `
	...
	for (const tmpl of fsTemplates) {
		try {
			...
			console.error(`[Server] Error syncing template ${tmpl.id}:`, error);
		}
	}

	console.log('[Server] Template sync completed.');
}
...
export async function seedAdmin(): Promise<void> {
	...
	// Gunakan env vars untuk kredensial admin, fallback ke default hanya untuk development
	const { env } = await import('$env/dynamic/private');
	const adminUsername = env.ADMIN_USERNAME || 'admin';
	const adminEmail = env.ADMIN_EMAIL || 'admin@wedding.com';
	const adminPassword = env.ADMIN_PASSWORD;

	if (!adminPassword) {
		console.error('[Server] ADMIN_PASSWORD env var tidak diset! Admin tidak akan dibuat.');
		console.error('[Server] Set ADMIN_PASSWORD di .env untuk membuat akun admin pertama.');
		return;
	}

	const { createUser } = await import('./users');
	await createUser(adminUsername, adminEmail, adminPassword, 'admin');
	console.log(`[Server] Admin account created: ${adminEmail}`);
}
