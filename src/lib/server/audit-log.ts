...
export async function getAuditLogs(limit: number = 100, offset: number = 0, search: string = '', actionFilter: string = ''): Promise<AuditEntry[]> {
	const db = await getDb();
	...
	if (search) {
		conditions.push('(email LIKE ? OR details LIKE ? OR user_id LIKE ?)');
		const like = `%${search}%`;
