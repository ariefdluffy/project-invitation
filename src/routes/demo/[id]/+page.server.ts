...
import type { Invitation } from '$lib/server/invitations';

export const load: PageServerLoad = async ({ params }) => {
	const db = await getDb();
	const [rows] = await db.execute('SELECT * FROM templates WHERE id = ?', [params.id]);
	let templates = rows as any[];

	// Jika tidak ketemu di database, coba cari di sistem template (file JSON)
	...
	if (templates.length === 0) {
		...
		if (jsonTemplate) {
			...
			const hasTitle = Boolean(current?.defaultContent?.title);
			if (!hasTitle) {
				templates[0] = {
					...current,
					...jsonTemplate,
	...
	// Random name pairs for variety
	const namePairs = [
		{ bride: 'Laras', brideFull: 'Larasati Putri, S.E', groom: 'Bayu', groomFull: 'Bayu Pamungkas, S.T' },
		{ bride: 'Maya', brideFull: 'Maya Sartika, M.Pd', groom: 'Rizky', groomFull: 'Rizky Ramadhan, B.A' },
		{ bride: 'Dewi', brideFull: 'Dewi Anggraeni, S.Kom', groom: 'Aditya', groomFull: 'Aditya Pratama, M.M' },
		{ bride: 'Anisa', brideFull: 'Anisa Rahmawati, S.Ked', groom: 'Gilang', groomFull: 'Gilang Permana, S.H' },
		{ bride: 'Putri', brideFull: 'Putri Handayani, S.Sos', groom: 'Dimas', groomFull: 'Dimas Setiawan, S.T' },
		{ bride: 'Indah', brideFull: 'Indah Lestari, A.Md', groom: 'Fajar', groomFull: 'Fajar Hidayat, M.Si' },
		{ bride: 'Siti', brideFull: 'Siti Aminah, S.Ag', groom: 'Ahmad', groomFull: 'Ahmad Fauzi, Lc' }
	];
	...
	const dummyInvitation = {
		...
		bank_accounts: JSON.stringify([{ bank: 'BCA', number: '1234567890', name: 'Indriana Putri' }]),
		dress_code_colors: JSON.stringify(['#8B6914', '#D4A574', '#FDF6E3']),
	...
	const normalizedInvitation = { ...dummyInvitation } as Record<string, unknown>;
	const assignIfPresent = (key: string, targetKey: string) => {
		const value = defaultContent[key];
		if (value === undefined || value === null || value === '') return;
		if (targetKey === 'bank_accounts' || targetKey === 'dress_code_colors') {
			normalizedInvitation[targetKey] = Array.isArray(value)
	...
	if (defaultContent.title) {
		normalizedInvitation.custom_content = JSON.stringify({
			title: String(defaultContent.title)
		});
	}

	return {
		template,
		invitation: normalizedInvitation as unknown as Invitation,
		wishes: [
			{ guest_name: 'Budi Santoso', message: 'Selamat menempuh hidup baru ya Indri & Adi! Semoga samawa.', is_attending: 'hadir', created_at: new Date().toISOString() },
			{ guest_name: 'Siska Amelia', message: 'Happy wedding! Maaf belum bisa hadir langsung.', is_attending: 'tidak_hadir', created_at: new Date().toISOString() }
		],
