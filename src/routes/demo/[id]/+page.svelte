<script lang="ts">
  ...
  import type { Invitation, Template } from "$lib/server/invitations";
  ...
  const typedForm = form as { success?: boolean; error?: string } | null;

  const invitation = $derived(data.invitation as unknown as Invitation);
  const template = $derived(data.template as unknown as Template);
  const wishes = $derived(data.wishes);
  const guestName = $derived(data.guestName);
  const category = $derived((template?.category || "pernikahan").toLowerCase());
      ...
      <main class="invitation-main animate-fade-in">
        ...
        <section class="section rsvp-sec reveal">
          ...
          <div class="form-container">
            {#if typedForm?.success}
              <div class="alert alert-success">
                Terima kasih atas doa dan ucapannya! 🎉
              </div>
            {/if}
            {#if typedForm?.error}
              <div class="alert alert-danger">{typedForm?.error}</div>
            {/if}
