...
import { getPromoCodeByCode } from '$lib/server/promo-codes';
import { createMidtransTransaction } from '$lib/server/midtrans';
import { buildMidtransOrderId } from '$lib/server/midtrans-order-id';
import { createPaymentTransaction, updatePaymentTransactionStatus } from '$lib/server/payment-transactions';
import { activatePremiumAccess, addGuestLimitToUser } from '$lib/server/users';
...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	if (amount === 0) {
		...
		try {
			// Store transaction record (langsung tandai success karena gratis)
			await createPaymentTransaction(
				locals.user.id,
				orderId,
				orderType as 'premium' | 'addon',
				0
			);

			// Activate user access dengan signature yang benar
			if (orderType === 'premium') {
				const durationDays = 30;
				const expiresAt = new Date();
				expiresAt.setDate(expiresAt.getDate() + durationDays);
				await activatePremiumAccess(locals.user.id, {
					invitationLimit: 5,
					guestLimit: 100,
					expiresAt
				});
			} else if (orderType === 'addon') {
				const guestQuantity = parseInt(
					(await getSetting('addon_guest_quantity')) || '50'
				);
				await addGuestLimitToUser(locals.user.id, guestQuantity);
			}

			// Tandai transaksi sebagai success (gratis = langsung sukses)
			await updatePaymentTransactionStatus(orderId, 'success');

			// Increment promo & log dipindahkan ke notification handler untuk paid flow.
			// Untuk free flow, increment di sini karena tidak ada notifikasi.
			if (promo) {
				const { incrementPromoCodeUsage } = await import('$lib/server/promo-codes');
			...
			// SvelteKit redirect throws a plain object with status & location
	...
	try {
		...
		// Store transaction record for webhook lookup (sertakan promo code untuk dipakai webhook)
		await createPaymentTransaction(
			locals.user.id,
			orderId,
			orderType as 'premium' | 'addon',
		...
		// NOTE: Promo code increment dipindahkan ke webhook (saat pembayaran sukses).
		// Di titik ini transaksi masih 'pending' - belum tentu user benar-benar bayar.
		// Pelacakan promo yang dipilih disimpan di order_id + payment_transactions.
		if (promo) {
			logAudit({
				action: 'promo_code.used',
				userId: locals.user.id,
				email: locals.user.email,
				details: `Promo ${promo.code} reserved for order ${orderId} (${packageName})`
			}).catch(() => {});
		}
