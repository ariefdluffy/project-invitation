...
import { updateUserPassword, getUserById, getUserByIdWithPassword } from '$lib/server/users';
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	// Jika admin, redirect ke halaman profil admin
...
export const actions: Actions = {
	updatePassword: async ({ request, locals }) => {
		...
		const userWithPassword = await getUserByIdWithPassword(locals.user.id);
