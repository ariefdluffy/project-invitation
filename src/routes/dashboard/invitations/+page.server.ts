...
export const actions: Actions = {
	duplicate: async ({ request, locals }) => {
		...
		// Verifikasi ownership sebelum duplikat
		const { getInvitationById } = await import('$lib/server/invitations');
		const original = await getInvitationById(id);
		if (!original) return fail(404, { error: 'Undangan tidak ditemukan.' });
		if (original.user_id !== locals.user.id && locals.user.role !== 'admin') {
		...
		if (locals.user.role !== 'admin' && existing.length >= locals.user.invitation_limit) {
