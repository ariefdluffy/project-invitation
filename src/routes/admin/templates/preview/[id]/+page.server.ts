...
import { getTemplateById } from '$lib/server/invitations';
...
export const load: PageServerLoad = async ({ params }) => {
	...
	if (!template) {
		throw error(404, 'Template tidak ditemukan');
	}
	...
	const dummyInvitation = {
		...
		bank_accounts: JSON.stringify([
			...
			{ bank: 'Mandiri', number: '0987654321', name: 'Santi Rahayu' }
		]),
		dress_code_colors: JSON.stringify(['#FFFFFF', '#D4AF37', '#2C2C2C']),
	...
	return {
		...
		wishes: [
			{ id: '1', guest_name: 'Andi', message: 'Selamat menempuh hidup baru ya Budi & Santi! Semoga sakinah mawaddah warahmah.', is_attending: 'hadir', created_at: new Date().toISOString() },
			{ id: '2', guest_name: 'Rina', message: 'Happy wedding! Maaf tidak bisa hadir karena sedang di luar kota.', is_attending: 'tidak_hadir', created_at: new Date().toISOString() }
		],
