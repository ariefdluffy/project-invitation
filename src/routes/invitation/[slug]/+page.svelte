<script lang="ts">
    ...
    // Parse Custom Content: Menangani kasus dimana data bisa berupa string JSON atau objek yang sudah di-parse
	...
	{#if isOpened}
		<main class="invitation-main animate-fade-in">
			...
			<section class="section hero-sec reveal">
				<div class="hero-box">
					<p class="subtitle" style="font-family: var(--f-fam); letter-spacing: 0.3em; text-transform: uppercase; font-size: 0.9rem; opacity: 0.8; margin-bottom: 1rem;">
    {templateTitle}
</p>
					<h2 class="names">
						{invitation.bride_name} <br />&<br />
			...
			<section class="section couple-sec">
				<div class="section-heading">
					<h2 class="title">{customContent.heading || 'Bride & Groom'}</h2>
					<p>
						{customContent.invitation_text || "Dengan memohon rahmat dan ridho Allah Subhanahu Wa Ta'ala, kami bermaksud mengundang Bapak/Ibu/Saudara/i untuk menghadiri acara pernikahan kami:"}
					</p>
				</div>
				...
				<div class="couple-card bride reveal delay-1">
					...
					></div>
					<h3 class="name" style="font-family: var(--f-fam);">{invitation.bride_name}</h3>
				...
				<div class="couple-card groom reveal delay-2">
					...
					></div>
					<h3 class="name" style="font-family: var(--f-fam);">{invitation.groom_name}</h3>
				...
				<div class="event-cards">
					<div class="event-card reveal delay-1">
						...
						<div class="event-details">
							<p>
								<strong
									>{formatDate(invitation.akad_date)}</strong
								>
							</p>
			...
			{#if invitation.love_story}
				<section class="section story-sec reveal">
					<div class="section-heading">
						...
						<p>
							Every love story in this universe is beautiful, but
							ours is my favorite.
						</p>
					</div>
			...
			{#if galleryImages.length > 0}
				<section class="section gallery-sec">
					<div class="section-heading">
						...
						<p>
							Sebuah kebahagiaan bagi kami untuk membagikan
							momen-momen indah ini.
						</p>
					</div>
					<div class="gallery-grid">
						{#each galleryImages as img}
							<div class="gallery-item reveal zoom-in" style="background-image: url('{img}');"></div>
						{/each}
					</div>
				</section>
			{/if}
			...
			{#if dressColors.length > 0}
				<section class="section dress-sec reveal">
					<div class="section-heading">
						...
						<p>
							Kami mengajak Bapak/Ibu/Saudara/i untuk menggunakan
							pakaian dengan palet warna berikut di hari istimewa
							kami.
						</p>
					</div>
			...
			<section class="section rsvp-sec reveal">
				<div class="section-heading">
					...
					<p>
						Mohon berkenan untuk memberikan doa & ucapan untuk
						mempelai serta konfirmasi kehadiran dengan mengisi form
						berikut:
					</p>
				</div>
						...
						{#each wishes as wish}
							<div class="wish-card">
								<div class="wish-head">
									...
									<span
										class="wish-badge"
										class:hadir={wish.is_attending ===
											"hadir"}
									>
										{wish.is_attending === "hadir"
											? "Hadir"
											: "Tidak Hadir"}
									</span>
								</div>
			...
			<footer class="invitation-footer">
				<div class="footer-static-text" style="max-width: 80%; margin: 0 auto 3rem; font-style: italic; opacity: 0.9; line-height: 1.6;">
				...
				<div class="hormat-kami" style="margin-bottom: 3rem;">
					<p style="font-size: 0.9rem; margin-bottom: 0.8rem; opacity: 0.8;">Hormat Kami,</p>
					<h3 style="font-family: var(--f-fam); font-size: 1.3rem; color: white; line-height: 1.6;">
...
<style>
	...
	.template-celestial-night .countdown-item .value { color: #ffd700; }
	...
	.template-royal-midnight .countdown-item .value { color: var(--s-col); }
	...
	.template-celestial-night .couple-card .name { color: #ffd700; }
	...
	/* MODERN LAYOUT */
	.layout-modern .hero-box { border-radius: 0; border: 4px solid var(--p-col); background: white; }
	.layout-modern .names { font-family: 'Montserrat', sans-serif !important; text-transform: uppercase; letter-spacing: 5px; font-weight: 900; }
	.layout-modern .couple-card { border-radius: 0; box-shadow: 10px 10px 0 var(--s-col); }
	.layout-modern .event-card { border: 2px solid var(--p-col); border-radius: 0; }

	/* ROMANTIC LAYOUT */
	.layout-romantic .hero-box { border-radius: 100px 100px 0 0; border: none; background: rgba(255,255,255,0.8); }
	.layout-romantic .names { font-family: 'Great Vibes', cursive !important; font-size: 4rem; }
	.layout-romantic .couple-card { border-radius: 50% 50% 0 0; overflow: hidden; }
	.layout-romantic .section-heading .title::after { content: '🌸'; margin-left: 10px; }

	/* LUXURY & ROYAL LAYOUT */
	.layout-luxury .invitation-container, .layout-royal .invitation-container { background: #000; color: #eee; }
	.layout-luxury .hero-box, .layout-royal .hero-box { border: 1px solid var(--s-col); background: linear-gradient(to bottom, #1a1a1a, #000); }
	.layout-luxury .names, .layout-royal .names { font-family: 'Playfair Display', serif !important; letter-spacing: 2px; }
	.layout-luxury .couple-card, .layout-royal .couple-card { border: 1px solid var(--s-col); background: #111; color: white; }
	.layout-luxury .event-card, .layout-royal .event-card { background: #1a1a1a; border-top: 3px solid var(--s-col); }

	/* VINTAGE LAYOUT */
	.layout-vintage .invitation-container { background: #f4ece1; }
	.layout-vintage .hero-box { background: #fff; border: 2px dashed #8d6e63; border-radius: 8px; }
	.layout-vintage .names { font-family: 'Special Elite', cursive !important; color: #5d4037; }
	.layout-vintage .couple-card { background: #fff; border: 1px solid #d7ccc8; transform: rotate(-2deg); }
	.layout-vintage .event-card { background: #fff; border-bottom: 5px solid #8d6e63; }

	/* CELESTIAL LAYOUT */
	.layout-celestial .invitation-container { background: #0f172a; color: #fff; }
	.layout-celestial .hero-box { background: rgba(15, 23, 42, 0.8); border: 1px solid rgba(148, 163, 184, 0.3); border-radius: 50%; width: 350px; height: 350px; display: flex; flex-direction: column; justify-content: center; margin: 0 auto; }
	.layout-celestial .names { font-family: 'Cinzel Decorative', serif !important; font-size: 2.5rem; text-shadow: 0 0 10px rgba(255,255,255,0.5); }
	.layout-celestial .couple-card { background: rgba(255,255,255,0.05); backdrop-filter: blur(10px); border-radius: 20px; }

	/* TROPICAL LAYOUT */
	.layout-tropical .invitation-container { background: #f0fdfa; }
	.layout-tropical .hero-box { border-left: 8px solid var(--p-col); background: white; padding-left: 2rem; }
	.layout-tropical .names { font-family: 'Outfit', sans-serif !important; color: var(--p-col); font-weight: 800; }
	.layout-tropical .couple-card { border-radius: 30px; background: white; box-shadow: 0 10px 30px rgba(6, 95, 70, 0.1); }
</style>
